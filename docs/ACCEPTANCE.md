...
- UI/报告源码乱码扫描
- C 盘写入目标扫描
- 音频波形/能量与短片段、视频预览帧步进、RTP sequence、RTCP SR/RR 会话质量和 Raw YUV 逐帧预览冒烟测试
- 绿色版 GUI 启动冒烟测试
...
8. 运行 `PowerShell -ExecutionPolicy Bypass -File G:\AVScope\scripts\validate_qt_ui.ps1`，确认 Qt 构建、协议树数据契约、深浅主题、时间线、RTCP 会话预览、媒体流表、诊断筛选、Raw PCM 波形、Raw YUV 第 1/2 帧、工程书签恢复、协议对比截图、高 DPI、异步任务约束和 G 盘状态文件检查通过；验证证据位于 `G:\AVScope\tmp\qt-ui-validation`。
