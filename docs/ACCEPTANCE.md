...
8. 运行 `PowerShell -ExecutionPolicy Bypass -File G:\AVScope\scripts\validate_qt_ui.ps1`，确认 Qt 构建、协议树数据契约、深浅主题、时间线、Raw PCM 波形、Raw YUV 第 1/2 帧、工程快照恢复、协议对比截图、高 DPI、异步任务约束和 G 盘状态文件检查通过；验证证据位于 `G:\AVScope\tmp\qt-ui-validation`。
...
13. 在工具栏搜索框输入 `RIFF`，检查 `Ctrl+F` 聚焦搜索框、`F3` 查找下一个、`Ctrl+G` 可按十进制或十六进制跳转 Offset、`Ctrl+1~6` 可切换工作区标签，并可复制当前 Offset、值和文件完整路径。
...
30. 使用“文件 / 保存工程快照”保存 `.avscope.json`，确认文件包含当前分析结果、源文件路径、Raw 参数、主题和当前标签页；关闭后通过“打开文件”重新打开该快照，协议树、Hex、诊断、主题和标签页应恢复，源文件移动时应提示但仍能离线查看分析结果。
