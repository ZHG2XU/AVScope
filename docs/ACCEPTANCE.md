...
- 导出 HTML/JSON/CSV 报告后，HTML 应包含“时间线曲线摘要”、时间线异常清单、PTS/DTS 曲线、码率曲线、GOP 结构图、RTP sequence 曲线、PCR 曲线和异常点，帧列表与 Packet 时间线的 `Issue` 列应显示异常原因，JSON/CSV 应包含 `timeline_summary`。
