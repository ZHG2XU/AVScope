...
5. 需要安装包验收时运行：`G:\AVScope\dist\AVScope-Setup.exe`，默认安装目录为 `C:\Program Files\AVScope`。
6. 需要重新验证时运行：
...
8. 运行 `PowerShell -ExecutionPolicy Bypass -File G:\AVScope\scripts\validate_qt_ui.ps1`，确认 Qt 构建、协议树数据契约、深浅主题、时间线、RTCP 会话预览与控制反馈、RTP 时序质量、TWCC 拥塞反馈、传输会话异常筛选、H.264/H.265 码流健康、媒体流表、诊断筛选、Raw PCM 波形、Raw YUV 第 1/2 帧、工程书签恢复、协议对比截图、高 DPI、异步任务约束和 G 盘状态文件检查通过；验证证据位于 `G:\AVScope\tmp\qt-ui-validation`。
...
20.4 打开 `G:\AVScope\samples\sample_sip_sdp.pcap`，进入“传输会话 / 信令协商”，确认显示 3 条 SIP、2 个 Call-ID、5 个原始 SDP 媒体描述和 4 条去重 PT 映射；PT 96 在端口 5004/6004 应分别映射为 H264/H265，PT 97 映射为 PCMA/8000 且不能误入视频负载。会话质量页应显示 SDP 关联 3，音频 RTP timestamp 8000 应换算为 1.0 秒；双击 SIP/SDP 行应定位 Hex。HTML 应包含“SIP / SDP 信令协商”，CSV 应包含 `sip_sdp_summary`、`sip_message` 和 `sdp_payload_mapping`。
20.5 打开 `G:\AVScope\samples\sample_rtcp_feedback.pcap`，进入“传输会话 / 控制反馈”，确认 NACK 1 次并展开丢失序号 101/103/105、PLI 1 次、FIR 1 次且 sequence 为 7、SDES CNAME 为 `camera-01@example`、BYE 原因为 `stream ended`；对应 `0x12345678` 会话应标记为警告。双击事件应定位 RTCP Hex。HTML 应包含“RTCP 控制反馈”，CSV 应包含 `rtcp_feedback`、`rtcp_sdes` 和 `rtcp_bye`。
20.6 打开 `G:\AVScope\samples\sample_rtp_timing.pcap`，进入“传输会话 / 时序质量”，确认 PT 8 使用 8000 Hz 静态 Clock Rate，4 包的 RFC 3550 Jitter 为 1.875 ms、平均到达间隔 30 ms、范围 20–50 ms、最大偏差 30 ms、突发 1 次；突发序号应为 103，双击事件定位 Offset `0x304`。HTML 应包含“RTP 时序质量”，CSV 应包含 `rtp_timing_session` 和 `rtp_timing_event`。
20.7 打开 `G:\AVScope\samples\sample_rtcp_twcc.pcap`，进入“传输会话 / 拥塞反馈”，确认 1 个 TWCC 反馈、Base Sequence 1000、5 个状态、收到 4、丢失 1、反馈计数 7、最大 Delta 30 ms；逐包表应显示序号 1002 未接收、序号 1003 Large Delta 30 ms。REMB 表应显示目标 `0xCAFEBABE`、指数 4、尾数 156250、估计带宽 2.5 Mbps。双击反馈、逐包或 REMB 行应定位 Hex。HTML 应包含“RTCP TWCC / REMB 拥塞反馈”，CSV 应包含 `rtcp_twcc`、`rtcp_twcc_packet` 和 `rtcp_remb`。
...
32. 打开 `G:\AVScope\dist\sample-reports\sample_wav_report.html`、`sample_mp4_report.json`、`sample_rtp_video_report.html`、`sample_sip_sdp_report.html`、`sample_rtcp_feedback_report.html`、`sample_rtp_timing_report.html` 和 `sample_rtcp_twcc_report.html`，确认交付目录包含文件、封装、RTP 视频负载、SIP/SDP、RTCP 控制反馈、RTP 时序质量与 TWCC 拥塞反馈示例报告。
...
36. 运行安装包，默认安装目录应为 `C:\Program Files\AVScope`，并可选择是否创建桌面和开始菜单快捷方式。
