...
8. 运行 `PowerShell -ExecutionPolicy Bypass -File G:\AVScope\scripts\validate_qt_ui.ps1`，确认 Qt 构建、协议树数据契约、深浅主题、时间线、RTCP 会话预览与控制反馈、RTP 时序质量、传输会话异常筛选、H.264/H.265 码流健康、媒体流表、诊断筛选、Raw PCM 波形、Raw YUV 第 1/2 帧、工程书签恢复、协议对比截图、高 DPI、异步任务约束和 G 盘状态文件检查通过；验证证据位于 `G:\AVScope\tmp\qt-ui-validation`。
...
32. 打开 `G:\AVScope\dist\sample-reports\sample_wav_report.html`、`sample_mp4_report.json`、`sample_rtp_video_report.html`、`sample_sip_sdp_report.html`、`sample_rtcp_feedback_report.html` 和 `sample_rtp_timing_report.html`，确认交付目录包含文件、封装、RTP 视频负载、SIP/SDP、RTCP 控制反馈与 RTP 时序质量示例报告。
