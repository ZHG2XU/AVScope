...
- 音频预览：增加声道选择和波形差异对比。
- 时间线：继续增加 RTP sequence 多流对齐视图和异常标记筛选。
...
- GUI 时间线页已叠加 PTS/DTS 曲线、码率曲线、GOP 分段、RTP sequence 曲线、PCR 曲线和异常标记，预览页会显示时间线曲线摘要；HTML/JSON/CSV 导出已包含同一份结构化数据，PCAP/RTP 还会显示 sequence 曲线与跳变点，MPEG-TS 会显示 PCR 曲线。
- 后续仍建议继续扩展 RTP sequence 多流对齐视图、异常筛选标记、后台索引、取消任务和 10GB 级自动化压力测试。
