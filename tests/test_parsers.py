...
class ParserTests(unittest.TestCase):
    ...
    def test_mp4_parser_and_reports(self):
        ...
        self.assertIn("AVScope", html_text)
        self.assertIn("媒体摘要", html_text)
        self.assertIn("用户备注", html_text)
        self.assertIn(report_notes, html_text)
        self.assertIn('class="hero-summary"', html_text)
        self.assertIn("grid-template-columns: repeat(2, minmax(0, 1fr))", html_text)
    ...
    def test_timeline_issue_rows_include_transport_warnings(self):
        ...
        self.assertIn("current=0.5", rows[1]["detail"])
        self.assertEqual(
            timeline_issue_item_orders(
                {
                    "rtp_sequence": {"warnings": [{"item_order": 4}]},
                    "pcr": {"warnings": [{"item_order": 6}, {"pid": "0x0200", "count": 1}]},
                }
            ),
            {4, 6},
        )
        ...
        pcr_summary = build_timeline_summary(
            [
                ...
                FrameInfo(index=1, offset=188, size=188, metadata={"pcr_pid": "0x0100", "pcr_seconds": 0.5}),
            ],
            [],
        )
