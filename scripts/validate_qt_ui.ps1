...
$env:PYTHONPATH = $root
& "E:\DevelopmentEnvironment\python\python.exe" -c "import json; from pathlib import Path; from avscope.analyzer import Analyzer; from avscope.report import export_json; source=Path(r'$root/samples/sample.mp4'); report=Path(r'$output/snapshot-analysis.json'); export_json(Analyzer().analyze(source),report); analysis=json.loads(report.read_text(encoding='utf-8')); snapshot={'schema_version':1,'source_path':str(source),'theme':'light','current_tab':0,'raw_options':[],'analysis':analysis}; Path(r'$snapshotPath').write_text(json.dumps(snapshot,ensure_ascii=False,indent=2),encoding='utf-8')"
