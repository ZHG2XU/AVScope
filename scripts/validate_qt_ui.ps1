...
& "E:\DevelopmentEnvironment\python\python.exe" -c "import json; from pathlib import Path; document=json.loads(Path(r'$twccJson').read_text(encoding='utf-8')); rtcp=document['media']['summary']['rtcp']; state=json.loads(Path(r'$twccState').read_text(encoding='utf-8')); assert state=={'events':1,'received':4,'lost':1,'max_delta_ms':30.0,'packets':5}, state; event=rtcp['twcc_events'][0]; assert event['base_sequence']==1000 and event['packets'][2]['sequence']==1002 and not event['packets'][2]['received']; assert event['packets'][3]['delta_ms']==30.0; print({'twcc':state,'lost_packet':event['packets'][2],'large_delta':event['packets'][3]})"
