...
$rtpTimingScreenshot = "$output\rtp-timing.png"
...
$rtpTimingProcess = Start-Process -FilePath $executable -ArgumentList "$root\samples\sample_rtp_timing.pcap" -WindowStyle Hidden -PassThru
if (-not $rtpTimingProcess.WaitForExit(10000)) {
    $rtpTimingProcess.Kill()
    throw "Qt RTP timing smoke test timed out"
}
if (-not (Test-Path -LiteralPath $rtpTimingScreenshot)) {
    throw "Qt RTP timing screenshot was not generated"
}
Copy-Item -LiteralPath "$root\tmp\qt-runtime\current-analysis.json" -Destination $rtpTimingJson -Force
Remove-Item Env:\AVSCOPE_START_TAB, Env:\AVSCOPE_TRANSPORT_DETAIL_TAB, Env:\AVSCOPE_RTP_TIMING_STATE -ErrorAction SilentlyContinue

$twccScreenshot = "$output\rtcp-twcc.png"
...
$twccProcess = Start-Process -FilePath $executable -ArgumentList "$root\samples\sample_rtcp_twcc.pcap" -WindowStyle Hidden -PassThru
if (-not $twccProcess.WaitForExit(10000)) {
    $twccProcess.Kill()
    throw "Qt TWCC congestion feedback smoke test timed out"
}
if (-not (Test-Path -LiteralPath $twccScreenshot)) {
    throw "Qt TWCC congestion feedback screenshot was not generated"
}
Copy-Item -LiteralPath "$root\tmp\qt-runtime\current-analysis.json" -Destination $twccJson -Force
Remove-Item Env:\AVSCOPE_START_TAB, Env:\AVSCOPE_TRANSPORT_DETAIL_TAB, Env:\AVSCOPE_TWCC_STATE -ErrorAction SilentlyContinue
...
$env:PYTHONPATH = $root
& $python -c "from pathlib import Path; source=Path(r'$root/samples/sample.mp4'); data=bytearray(source.read_bytes()); marker=data.index(b'stco'); offset=marker+12; data[offset:offset+4]=(8).to_bytes(4,'big'); Path(r'$diagnosticSample').write_bytes(data)"
...
& $python -c "import json; from pathlib import Path; state=json.loads(Path(r'$singleHexState').read_text(encoding='utf-8')); expected={'path':str(Path(r'$root/samples/sample.yuv')),'file_size':13824,'page_size':4096,'page_index':1,'page_count':4,'start_offset':4096,'end_offset':8191,'previous_enabled':True,'next_enabled':True}; assert state==expected, state; print({'single_hex_pagination':state})"
...
& $python -c "import json; from pathlib import Path; document=json.loads(Path(r'$rtpTimingJson').read_text(encoding='utf-8')); transport=document['media']['summary']['transport_sessions']; state=json.loads(Path(r'$rtpTimingState').read_text(encoding='utf-8')); assert state=={'sessions':1,'warnings':1,'max_jitter_ms':1.875,'max_deviation_ms':30.0,'bursts':1}, state; timing=transport['sessions'][0]['rtp_timing']; assert timing['clock_rate']==8000 and timing['clock_source']=='RTP static PT' and timing['events'][0]['sequence']==103, timing; assert transport['sessions'][0]['status']=='warning'; print({'rtp_timing':state,'clock':timing['clock_rate'],'event':timing['events'][0]})"
if ($LASTEXITCODE -ne 0) { throw "Qt RTP timing quality page contract is incomplete" }

& $python -c "import json; from pathlib import Path; document=json.loads(Path(r'$twccJson').read_text(encoding='utf-8')); rtcp=document['media']['summary']['rtcp']; state=json.loads(Path(r'$twccState').read_text(encoding='utf-8')); assert state=={'events':1,'received':4,'lost':1,'max_delta_ms':30.0,'packets':5,'remb':1,'remb_min_bps':2500000}, state; event=rtcp['twcc_events'][0]; assert event['base_sequence']==1000 and event['packets'][2]['sequence']==1002 and not event['packets'][2]['received']; assert event['packets'][3]['delta_ms']==30.0; remb=rtcp['remb_events'][0]; assert remb['bitrate_bps']==2500000 and remb['target_ssrcs']==[0xCAFEBABE]; print({'twcc':state,'lost_packet':event['packets'][2],'large_delta':event['packets'][3],'remb':remb})"
if ($LASTEXITCODE -ne 0) { throw "Qt TWCC congestion feedback page contract is incomplete" }
