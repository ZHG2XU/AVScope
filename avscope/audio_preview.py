...
def build_audio_preview_clip(
    ...
    target = audio_preview_output_path(source, target_dir, duration)
    try:
        if format_name == "WAV" or source.suffix.lower() == ".wav":
            return _wav_clip(source, target, duration)
        if format_name == "Raw PCM" or source.suffix.lower() == ".pcm":
            return _pcm_clip(source, target, media_summary, duration)
        return _ffmpeg_audio_clip(source, target, duration)
...
def audio_preview_output_path(source: Path, output_dir: Path, duration_seconds: float) -> Path:
    try:
        stat = source.stat()
        marker = f"{source.resolve()}|{stat.st_size}|{stat.st_mtime_ns}|{duration_seconds:.3f}"
    except OSError:
        marker = f"{source}|{duration_seconds:.3f}"
    ...
    return output_dir / f"{stem}-{digest}.wav"
...
def _wav_clip(source: Path, target: Path, duration_seconds: float) -> dict[str, Any]:
    with wave.open(str(source), "rb") as src:
        params = src.getparams()
        frames = min(src.getnframes(), max(1, int(src.getframerate() * duration_seconds)))
        data = src.readframes(frames)
    with wave.open(str(target), "wb") as dst:
        dst.setparams(params)
        dst.writeframes(data)
    return _clip_result(target, frames, params.framerate, params.nchannels)


def _pcm_clip(source: Path, target: Path, media_summary: dict[str, Any], duration_seconds: float) -> dict[str, Any]:
    ...
    frames = min(source.stat().st_size // frame_size, max(1, int(sample_rate * duration_seconds)))
    ...
    with wave.open(str(target), "wb") as dst:
        ...
        dst.writeframes(data)
    return _clip_result(target, frames, sample_rate, channels)


def _ffmpeg_audio_clip(source: Path, target: Path, duration_seconds: float) -> dict[str, Any]:
    exe = find_ffmpeg()
    if not exe:
    ...
    with wave.open(str(target), "rb") as wav:
        return _clip_result(target, wav.getnframes(), wav.getframerate(), wav.getnchannels())
...
def _clip_result(target: Path, frames: int, sample_rate: int, channels: int) -> dict[str, Any]:
    return {
        "available": True,
        "path": str(target),
        "frames": frames,
